...
public class ArenaConsumer {
    ...
    private static final Logger log = LoggerFactory.getLogger(ArenaConsumer.class);
    private static final String PROPERTIES_FILE = "consumer.properties";

    private final KafkaConsumer<String, String> consumer;
    private final AtomicBoolean running = new AtomicBoolean(true);

    public ArenaConsumer(String topic) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(this::shutdown, "arena-consumer-shutdown"));
    }
    ...
    public void run() {
        log.info("🏁 ArenaConsumer started. Waiting for records...");
        ...
        consumer.close();
        log.info("✅ ArenaConsumer stopped. Total records consumed = {}", totalCount);
    }
    ...
    private void shutdown() {
        log.info("Shutdown requested for ArenaConsumer...");
        running.set(false);
    }
    ...
    public static void main(String[] args) {
        String topic = (args.length > 0) ? args[0] : "arena-default";
        ArenaConsumer consumer = new ArenaConsumer(topic);
        consumer.run();
    }
}
