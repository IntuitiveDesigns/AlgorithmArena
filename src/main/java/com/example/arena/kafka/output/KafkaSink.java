...
public final class KafkaSink implements OutputSink<String>, AutoCloseable {
    ...
    public KafkaSink(
        ...
        // Logging interval
        this.errorLogIntervalMs = Math.max(100L, parseLong(props.getProperty("arena.sink.error.log.interval.ms", "1000"), 1000L));
        ...
        if (registry != null) {
            this.okCounter = registry.counter("arena_kafka_send_ok_total", "pipeline", this.pipelineName, "topic", this.topic);
            this.failCounter = registry.counter("arena_kafka_send_fail_total", "pipeline", this.pipelineName, "topic", this.topic);
            this.sendLatencyTimer = registry.timer("arena_kafka_send_latency", "pipeline", this.pipelineName, "topic", this.topic);
        } else {
            this.okCounter = null;
    ...
    public static KafkaSink fromConfig(PipelineConfig config, String topic, MetricsRuntime metrics) {
        ...
        String pipelineName = config.getProperty("pipeline.name", "AlgorithmArena");
        ...
        props.putIfAbsent("arena.sink.error.log.interval.ms", config.getProperty("arena.sink.error.log.interval.ms", "1000"));
